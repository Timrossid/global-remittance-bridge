...
export interface IAnchorAdapter {
  ...
  initiateDeposit(userId: string, amount: number, asset: string): Promise<{ depositUrl: string }>;
  initiateWithdrawal(userId: string, amount: number, asset: string, destination: string): Promise<{ txId: string }>;
  checkKycStatus(userId: string): Promise<AnchorKycStatus>;
  handleWebhook(payload: any): Promise<{ status: string }>;
}
